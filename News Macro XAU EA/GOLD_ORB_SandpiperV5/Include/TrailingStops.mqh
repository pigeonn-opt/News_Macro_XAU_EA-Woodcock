...
bool CTrailing::TrailingStop(string pSymbol,int pTrailPoints,int pMinProfit=0,int pStep=10)
{
	if(PositionSelect(pSymbol) == true && pTrailPoints > 0)
	{
			...
			if(DebugPrint) PrintFormat("[CTrailing] TrailingStopPoints(symbol=%s,pTrailPoints=%d,pMinProfit=%d,pStep=%d)", pSymbol, pTrailPoints, pMinProfit, pStep);

		if(DebugPrint) PrintFormat("[CTrailing] TrailingStop(symbol=%s,pTrailPoints=%d,pMinProfit=%d,pStep=%d)", pSymbol, pTrailPoints, pMinProfit, pStep);
		...
		do 
		{
			if(posType == POSITION_TYPE_BUY)
			{
				...
				currentProfit = SymbolInfoDouble(pSymbol,SYMBOL_BID) - openPrice;
					if(DebugPrint) PrintFormat("[CTrailing] BUY trailStopPrice=%.5f currentStop=%.5f step=%.5f currentProfit=%.5f minProfit=%.5f",
							trailStopPrice, currentStop, step, currentProfit, minProfit);
					if(trailStopPrice > currentStop + step && currentProfit >= minProfit)
					{
						request.sl = trailStopPrice;
						if(DebugPrint) PrintFormat("[CTrailing] Sending OrderSend for BUY SL=%.5f", request.sl);
						bool sent = OrderSend(request,result);
					}
					else
					{
						if(DebugPrint) PrintFormat("[CTrailing] BUY conditions not met, no OrderSend");
						return(false);
					}
			}
			else if(posType == POSITION_TYPE_SELL)
			{
				...
				currentProfit = openPrice - SymbolInfoDouble(pSymbol,SYMBOL_ASK);
					if(DebugPrint) PrintFormat("[CTrailing] SELL trailStopPrice=%.5f currentStop=%.5f step=%.5f currentProfit=%.5f minProfit=%.5f",
							trailStopPrice, currentStop, step, currentProfit, minProfit);
					if((trailStopPrice < currentStop - step || currentStop == 0) && currentProfit >= minProfit)
					{ 
						request.sl = trailStopPrice;
						if(DebugPrint) PrintFormat("[CTrailing] Sending OrderSend for SELL SL=%.5f", request.sl);
						bool sent = OrderSend(request,result);
					}
					else
					{
						if(DebugPrint) PrintFormat("[CTrailing] SELL conditions not met, no OrderSend");
						return(false);
					}
			}
...
bool CTrailing::TrailingStop(ulong pTicket,int pTrailPoints,int pMinProfit=0,int pStep=10)
{
	if(PositionSelectByTicket(pTicket) == true && pTrailPoints > 0)
	{
		...
		{
			PrintFormat("Skipping trailing stop update for closed position #%I64u", pTicket);
			return(false);
		}
		string symbol = PositionGetString(POSITION_SYMBOL);
		if(symbol == "")
		{
			PrintFormat("Skipping trailing stop update for position #%I64u due to empty symbol", pTicket);
			return(false);
		}
...
bool CTrailing::BreakEven(ulong pTicket,int pBreakEven,int pLockProfit=0)
{
	if(PositionSelectByTicket(pTicket) == true && pBreakEven > 0)
	{
		...
		if(!PositionSelectByTicket(pTicket))
		{
			PrintFormat("Skipping break-even update for closed position #%I64u", pTicket);
			return(false);
		}
		string symbol = PositionGetString(POSITION_SYMBOL);
		if(symbol == "")
		{
			PrintFormat("Skipping break-even update for position #%I64u due to empty symbol", pTicket);
			return(false);
		}
		request.symbol = symbol;
		double currentTP = PositionGetDouble(POSITION_TP);
