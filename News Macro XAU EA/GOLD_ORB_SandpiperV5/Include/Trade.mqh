...
protected:
   ...
   MqlTradeRequest   request;
    ...
    bool OpenPending(string pSymbol, ENUM_ORDER_TYPE pType, double pVolume, 
                     double pPrice, double pStop = 0, double pProfit = 0, 
...
bool CTrade::OpenPosition(string pSymbol, ENUM_ORDER_TYPE pType, double pVolume, double pStop, double pProfit, string pComment = NULL)
  {
   ZeroMemory(request);
   ZeroMemory(result);
   ...
   int filling = (int)SymbolInfoInteger(pSymbol,SYMBOL_FILLING_MODE);
      ...
      if(pType == ORDER_TYPE_BUY)
        {
         ...
         if(pStop > 0)
            request.sl = NormalizeDouble(request.price - (pStop * point),digits);
         if(pProfit > 0)
            request.tp = NormalizeDouble(request.price + (pProfit * point),digits);
...
// Open pending order
bool CTrade::OpenPending(string pSymbol,ENUM_ORDER_TYPE pType,double pVolume,double pPrice,double pStop, double pProfit, double pStopLimit,
                         datetime pExpiration, string pComment)
  {
   ZeroMemory(request);
   ZeroMemory(result);
