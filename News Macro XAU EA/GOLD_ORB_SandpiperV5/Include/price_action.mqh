...
protected:
   ...
   string m_blockedWeekDays;    // 新增
   string m_blockedHours;       // 新增
...
public:
   ...
   void                     Init();
   void SetBlockedWeekDays(string days) { m_blockedWeekDays = days; }
   void SetBlockedHours(string hours) { m_blockedHours = hours; }
   bool IsTradingBlocked();
   
private:
    // 辅助方法声明
    bool IsWeekDayBlocked(int dayOfWeek);
    bool IsHourBlocked(int hour);
    int SplitString(string str, string &result[], string delimiter);
  };
...
//| 字符串分割辅助函数                                               |
//+------------------------------------------------------------------+
int Price_Action::SplitString(string str, string &result[], string delimiter)
{
    int count = 0;
    int pos = 0;
    int found = StringFind(str, delimiter, pos);
    
    while(found >= 0)
    {
        ArrayResize(result, count + 1);
        result[count] = StringSubstr(str, pos, found - pos);
        count++;
        pos = found + StringLen(delimiter);
        found = StringFind(str, delimiter, pos);
    }
...
//| 检查星期是否被禁止                                               |
//+------------------------------------------------------------------+
bool Price_Action::IsWeekDayBlocked(int dayOfWeek)
{
    // 使用 StringLen 替代 StringIsEmpty
    if(StringLen(m_blockedWeekDays) == 0)
        return(false);
    
    string days[];
    int count = SplitString(m_blockedWeekDays, days, ",");
    
    for(int i = 0; i < count; i++)
    {
        // 去除可能的空格
        StringTrimLeft(days[i]);
        StringTrimRight(days[i]);
        
        if((int)StringToInteger(days[i]) == dayOfWeek)
            return(true);
    }
    return(false);
}
...
//| 检查小时是否被禁止                                               |
//+------------------------------------------------------------------+
bool Price_Action::IsHourBlocked(int hour)
{
    // 使用 StringLen 替代 StringIsEmpty
    if(StringLen(m_blockedHours) == 0)
        return(false);
    
    string hours[];
    int count = SplitString(m_blockedHours, hours, ",");
    ...
    for(int i = 0; i < count; i++)
    {
        ...
        if((int)StringToInteger(hours[i]) == hour)
            return(true);
    }
    return(false);
}
...
//| 检查当前时间是否被禁止交易                                       |
//+------------------------------------------------------------------+
bool Price_Action::IsTradingBlocked()
{
    MqlDateTime now;
    TimeToStruct(TimeCurrent(), now);
    
    // 检查星期是否被禁止
    if(IsWeekDayBlocked(now.day_of_week))
        return(true);
    
    // 检查小时是否被禁止
    if(IsHourBlocked(now.hour))
        return(true);
